...
const ui = { modal: null, notice: null };
let state = loadState();
let relayStatus = "未连接";
...
function renderHome(displayName) {
  ...
  return `<section class="home-view">
    ...
    <div class="identity-card">
      <div><span class="label">本机身份</span><strong>${escapeHtml(displayName)}</strong><small>身份密钥只保存在这台设备的浏览器里</small></div>
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-action='show-invite']").forEach((node) => node.addEventListener("click", () => { ui.modal = { type: "invite", contactId: node.dataset.contact }; render(); }));
