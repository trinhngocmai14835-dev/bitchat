...
- 本机身份会单独保存，正常刷新或重新打开不会变化。双方只使用 6 位数字邀请码配对；对方发来第一条消息后会自动完成回传配对。
- Cloudflare Pages 部署使用同源 `/relay` 中继入口，手机不需要直接访问 `workers.dev` 域名；GitHub Pages 仍会回退到项目 Worker 地址。
...
1. A 点“新建邀请”，把 6 位数字邀请码发给 B。
2. B 点“导入邀请”，输入数字并点击“加入”，然后直接发送第一条消息。
3. A 收到第一条消息后会自动保存 B 的配对公钥，之后双方即可双向聊天。

这不是传统账号系统：身份密钥保存在当前浏览器的本地存储中，并额外保存了一份本机身份副本。清理浏览器网站数据、使用隐私浏览模式或更换浏览器容器仍可能丢失本机身份和联系人，因此当前版本不要把它当作密钥备份方案。
