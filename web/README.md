...
- 本机身份会单独保存，正常刷新或重新打开不会变化。双方可以使用 6 位数字邀请码、分享链接，或摄像头扫描 QR 邀请配对。
...
4. 两部手机各设置一个本机名称。A 创建邀请并发送 6 位数字邀请码，B 输入数字自动导入后显示回传邀请，A 再导入回传邀请。
5. 保持默认的 `nostr://public` 中继即可开始远程聊天。右上角菜单里的“删除双方记录”会向对方发送加密删除指令。

## 配对方式

1. A 点“新建邀请”，把 6 位数字邀请码发给 B；也可以发送分享链接、QR 或邀请文本。
2. B 点“导入邀请”，输入数字并点击“加入”；然后点“显示我的回传邀请”，把 B 的数字邀请码或分享链接发回 A。
